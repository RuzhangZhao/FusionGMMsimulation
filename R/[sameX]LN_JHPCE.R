...
for(id1 in c(id1s)){
        for(id3 in c(id3s)){
            ...
            for(i in (length(rr_gap)+1):100){
                ...
                runintgmm<-function(y_int,X_int,A_int,sum_uni,sum_mul){
                    fuse_uni<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "lasso",summary_type = "uni",use_sparseC = use_sparseC)
                    fuse_uni_1lam<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "lasso",summary_type = "uni",tune_ratio = F,use_sparseC = use_sparseC)
                    cat("20%")
                    fuse_uni_ada<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",use_sparseC = use_sparseC)
                    fuse_uni_ada_1lam<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",tune_ratio = F,use_sparseC = use_sparseC)
                    cat("40%")
                    fuse_mul<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "lasso",summary_type = "multi",use_sparseC = use_sparseC)
                    fuse_mul_1lam<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "lasso",summary_type = "multi",tune_ratio = F,use_sparseC = use_sparseC)
                    cat("60%")
                    fuse_mul_ada<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",use_sparseC = use_sparseC)
                    fuse_mul_ada_1lam<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",tune_ratio = F,use_sparseC = use_sparseC)
                    cat("80%\n")
                    r0<-R2(y0,M0%*%coefXA)
                ...
                rr_gap[[i]]<-runintgmm(y_int,X_int,A_int,sum_uni,sum_mul)
                rr_gap2[[i]]<-runintgmm(y_int,X_int,A_int,sum_uni1,sum_mul1)
