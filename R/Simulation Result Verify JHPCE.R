...
for (id1 in c(1:2)){
    ...
    for(id2 in c(1:2)){
        ...
        for(id3 in c(1:length(n_int_list))){
            ...
            if(!file.exists(paste0('/users/rzhao1/intgmm/result',extra_fold,'/',lnlg,'/',lnlg,'_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))){
                skipid<-c(skipid,id3)
            }else{
                if(id3<5){
                    extra_fold<-"_desparseC"
                    res<-readRDS(paste0('/users/rzhao1/intgmm/result',extra_fold,'/',lnlg,'/',lnlg,'_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
        ...
        gg<-ggplot(data = df)+
            ...
            annotate("text", x = 20, y = highbase*1.015, label = paste0("Max Achievable ",AUCR2),size=3)+
            #annotate("text", x = 26, y = highbase*1.005, label = paste0("Max Achievable ",AUCR2),size=3)+
        ...
        ggsave(paste0("/users/rzhao1/intgmm/plot/plot",lnlg,extra_fold,"/",gname,":",AUCR2,":Comp:Incomp=1:",inv_r,";RiskFactor:",p_X,".png"),gg,width = 7,height = 5)
        gg<-ggplot(data = df2)+
            ...
            theme_bar()+
            scale_x_continuous(breaks = sqrt(n_int_list),labels =n_int_list,name = "Internal Sample Size")
        ggsave(paste0("/users/rzhao1/intgmm/plot/plot",lnlg,extra_fold,"/",gname,":BetaMSE:Comp:Incomp=1:",inv_r,";RiskFactor:",p_X,".png"),gg,width = 7,height = 5)
    }
}
