...
for(id1 in c(id1s)){
    for(id2 in c(id2s)){
        for(id3 in c(id3s)){
            ...
            if(!file.exists(paste0('/users/rzhao1/intgmm/result',foldname,'/Lg/Lg_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))){
                rr_gap<-list()
                rr_gap2<-list()
            }else{
                rr_gap<-readRDS(paste0('/users/rzhao1/intgmm/result',foldname,'/Lg/Lg_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
                rr_gap2<-readRDS(paste0('/users/rzhao1/intgmm/result',foldname,'/Lg/LgAP_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
            }
            ...
            for(i in (length(rr_gap)+1):50){
                ...
                fuse_uni<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "lasso",summary_type = "uni",family="binomial",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_uni_1lam<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "lasso",summary_type = "uni",family="binomial",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("20%")
                fuse_uni_ada<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",family="binomial",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_uni_ada_1lam<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",family="binomial",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("40%")
                fuse_mul<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "lasso",summary_type = "multi",family="binomial",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_mul_1lam<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "lasso",summary_type = "multi",family="binomial",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("60%")
                fuse_mul_ada<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",family="binomial",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_mul_ada_1lam<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",family="binomial",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("80%")
                ...
                runi<-RC(M0%*%fuse_uni$beta[-length(fuse_uni$beta)])
                runi1<-RC(M0%*%fuse_uni_1lam$beta[-length(fuse_uni$beta)])
                runiada<-RC(M0%*%fuse_uni_ada$beta[-length(fuse_uni$beta)])
                runiada1<-RC(M0%*%fuse_uni_ada_1lam$beta[-length(fuse_uni$beta)])

                rmul<-RC(M0%*%fuse_mul$beta[-length(fuse_uni$beta)])
                rmul1<-RC(M0%*%fuse_mul_1lam$beta[-length(fuse_uni$beta)])
                rmulada<-RC(M0%*%fuse_mul_ada$beta[-length(fuse_uni$beta)])
                rmulada1<-RC(M0%*%fuse_mul_ada_1lam$beta[-length(fuse_uni$beta)])
                ...
                betagap<-c(sum((coef(fuse_lasso,s="lambda.min")[-1] - coefXA)^2),
                           ...
                           sum((fuse_mul$beta[-length(fuse_uni$beta)] - coefXA)^2),
                           sum((fuse_mul_1lam$beta[-length(fuse_uni$beta)] - coefXA)^2),
                           sum((fuse_mul_ada$beta[-length(fuse_uni$beta)] - coefXA)^2),
                           sum((fuse_mul_ada_1lam$beta[-length(fuse_uni$beta)] - coefXA)^2),
                           sum((fuse_uni$beta[-length(fuse_uni$beta)] - coefXA)^2),
                           sum((fuse_uni_1lam$beta[-length(fuse_uni$beta)] - coefXA)^2),
                           sum((fuse_uni_ada$beta[-length(fuse_uni$beta)] - coefXA)^2),
                           sum((fuse_uni_ada_1lam$beta[-length(fuse_uni$beta)] - coefXA)^2)
                )
                ...
                # approximation
                fuse_uni<-cv.intgmm(y_int_g,X_int,A_int,sum_uni,penalty_type = "lasso",summary_type = "uni",family="gaussian",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_uni_1lam<-cv.intgmm(y_int_g,X_int,A_int,sum_uni,penalty_type = "lasso",summary_type = "uni",family="gaussian",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("20%")
                fuse_uni_ada<-cv.intgmm(y_int_g,X_int,A_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",family="gaussian",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_uni_ada_1lam<-cv.intgmm(y_int_g,X_int,A_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",family="gaussian",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("40%")
                fuse_mul<-cv.intgmm(y_int_g,X_int,A_int,sum_mul,penalty_type = "lasso",summary_type = "multi",family="gaussian",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_mul_1lam<-cv.intgmm(y_int_g,X_int,A_int,sum_mul,penalty_type = "lasso",summary_type = "multi",family="gaussian",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("60%")
                fuse_mul_ada<-cv.intgmm(y_int_g,X_int,A_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",family="gaussian",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_mul_ada_1lam<-cv.intgmm(y_int_g,X_int,A_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",family="gaussian",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("80%")
                ...
                ratios<-c(fuse_uni$ratio_min,fuse_uni_ada$ratio_min,fuse_mul$ratio_min,
                          fuse_mul_ada$ratio_min)
                print(round(ratios,3))
                rr_gap2[[i]]<-list("rr"=rr,"beta"=betagap,"ratios"=ratios)

                if (i %% 5 == 0){
                    saveRDS(rr_gap,paste0('/users/rzhao1/intgmm/result',foldname,'/Lg/Lg_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
                    saveRDS(rr_gap2,paste0('/users/rzhao1/intgmm/result',foldname,'/Lg/LgAP_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
                }
            }
        }}}
