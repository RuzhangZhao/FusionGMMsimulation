...
for(id1 in c(id1s)){
    for(id2 in c(id2s)){
        for(id3 in c(id3s)){
            ...
            if(!file.exists(paste0('/users/rzhao1/intgmm/result_use_sparseC/Ln/Ln_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))){
                rr_gap<-list()
            }else{
                rr_gap<-readRDS(paste0('/users/rzhao1/intgmm/result_use_sparseC/Ln/Ln_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
            }
            ...
            for(i in (length(rr_gap)+1):50){
                ...
                X_int<-M_int[,1:p_X]
                A_int<-M_int[,-c(1:p_X)]
                fuse_uni<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "lasso",summary_type = "uni",family="gaussian",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_uni_1lam<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "lasso",summary_type = "uni",family="gaussian",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("20%")
                fuse_uni_ada<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",family="gaussian",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_uni_ada_1lam<-cv.intgmm(y_int,X_int,A_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",family="gaussian",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("40%")
                fuse_mul<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "lasso",summary_type = "multi",family="gaussian",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_mul_1lam<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "lasso",summary_type = "multi",family="gaussian",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("60%")
                fuse_mul_ada<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",family="gaussian",use_sparseC = use_sparseC,seed.use=seed.use)
                fuse_mul_ada_1lam<-cv.intgmm(y_int,X_int,A_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",family="gaussian",tune_ratio = F,use_sparseC = use_sparseC,seed.use=seed.use)
                cat("80%")
                ...
                if (i %% 5 == 0){
                    saveRDS(rr_gap,paste0('/users/rzhao1/intgmm/result_use_sparseC/Ln/Ln_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
                }
            }
        }}}
