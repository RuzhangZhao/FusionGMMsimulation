...
fuse_uni_1lam<-fusionGMM.addition(X_int,A_int,y_int,sum_uni,penalty_type = "lasso",summary_type = "uni",inference = T,tune_ratio = F)
fuse_mul_1lam<-fusionGMM.addition(X_int,A_int,y_int,sum_mul,penalty_type = "lasso",summary_type = "multi",inference = T,tune_ratio = F)

fuse_lasso<-cv.glmnet(x = M_int,y = y_int)
#which(coef(fuse_lasso,s="lambda.1se")[-1]!=0)
...
print(fuse_uni_1lam$corrected_pos)
print(fuse_mul_1lam$corrected_pos)
print(c(sum(!fuse_uni_1lam$corrected_pos%in%which(coefAX!=0))/length(fuse_uni_1lam$corrected_pos),
        sum(!fuse_mul_1lam$corrected_pos%in%which(coefAX!=0))/length(fuse_mul_1lam$corrected_pos) ))
