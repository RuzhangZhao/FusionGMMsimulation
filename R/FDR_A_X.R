...
# large X small A so use previous A as X why we call A_X
...
for(i in 1:100){
    ...
    y0<-M0%*%coefAX+intercept+rnorm(nrow(M0),0,3)
    R2(y0,M0%*%coefAX)
    ...
    fuse_uni_ada<-fusionGMM.addition(X_int,A_int,y_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",inference = T)
    fuse_uni_ada_1lam<-fusionGMM.addition(X_int,A_int,y_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",tune_ratio = F,inference = T)

    fuse_mul_ada<-fusionGMM.addition(X_int,A_int,y_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",inference = T)
    fuse_mul_ada_1lam<-fusionGMM.addition(X_int,A_int,y_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",tune_ratio = F,inference = T)
    ...
    print(nonpos)
    if (i %% 5 == 0){
        saveRDS(nonpostotal,paste0(path,'pos_ir_',inv_r,'_pX_',p_A,'.rds'))
    }
}
