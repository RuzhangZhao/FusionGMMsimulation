...
for(i in 1:100){
    ...
    #fuse_uni_ada<-cv.htlgmm(y_int,X_int,A_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",family = "gaussian",tune_ratio = T,inference = T,use_sparseC=use_sparseC)
    fuse_uni_ada_1lam<-cv.htlgmm(y_int,X_int,A_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",family = "gaussian",tune_ratio = F,inference = T,use_sparseC=use_sparseC)
    fuse_uni_ada = fuse_uni_ada_1lam
    #fuse_mul_ada<-cv.htlgmm(y_int,X_int,A_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",family = "gaussian",inference = T,use_sparseC=use_sparseC)
    fuse_mul_ada_1lam<-cv.htlgmm(y_int,X_int,A_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",family = "gaussian",tune_ratio = F,inference = T,use_sparseC=use_sparseC)
    fuse_mul_ada = fuse_mul_ada_1lam
    ...
    www[is.infinite(www)]<-max(www[!is.infinite(www)])*100
    fuse_ada2<-cv.glmnet(x = M_int,y = y_int,penalty.factor = www)
    ...
    ada_res_lm<-ada_res_func(fuse_ada2)

    fdrgmm<-function(fuse_uni_ada){
        sum(!fuse_uni_ada$selected_vars$FDR_adjust_position%in%correct_pos)/length(fuse_uni_ada$selected_vars$FDR_adjust_position)
    }
    fdrgmm0<-function(fuse_uni_ada){
        cp0<-fuse_uni_ada$selected_vars$position[fuse_uni_ada$selected_vars$pval<0.05]
        sum(!cp0%in%correct_pos)/length(cp0)
    }
    power<-function(fuse_uni_ada){
        sum(fuse_uni_ada$selected_vars$FDR_adjust_position%in%correct_pos)/length(correct_pos)
    }
    ...
    cover<-function(fuse_uni_ada){
        ...
        for(i in 1:length(correct_pos)){
            ...
            if( fuse_uni_ada$beta[pos]!=0){
                nonzerovar<-fuse_uni_ada$selected_vars$final_v
                names(nonzerovar)<-fuse_uni_ada$selected_vars$position
    ...
    nonpos<-c(fdrgmm(ada_res),fdrgmm(ada_res_lm),fdrgmm(fuse_uni_ada),fdrgmm(fuse_uni_ada_1lam),fdrgmm(fuse_mul_ada),fdrgmm(fuse_mul_ada_1lam))
    #nonpos0<-c(fdrgmm0(ada_res),fdrgmm0(fuse_uni_ada),fdrgmm0(fuse_uni_ada_1lam),fdrgmm0(fuse_mul_ada),fdrgmm0(fuse_mul_ada_1lam))
    powers<-c(power(ada_res),power(ada_res_lm),power(fuse_uni_ada),power(fuse_uni_ada_1lam),power(fuse_mul_ada),power(fuse_mul_ada_1lam))
    #powers0<-c(power0(ada_res),power0(fuse_uni_ada),power0(fuse_uni_ada_1lam),power0(fuse_mul_ada),power0(fuse_mul_ada_1lam))
    coverage<-c(cover(ada_res),cover(ada_res_lm),cover(fuse_uni_ada),cover(fuse_uni_ada_1lam),cover(fuse_mul_ada),cover(fuse_mul_ada_1lam))
    allname<-c("ada","ada_lm","uni_ada","uni_ada_1lam","mul_ada","mul_ada_1lam")

    pval_list<-list()
    pval_list[[1]]<-ada_res$pval
    pval_list[[2]]<-ada_res_lm$selected_vars$pval
    pval_list[[3]]<-fuse_uni_ada$selected_vars$pval
    pval_list[[4]]<-fuse_uni_ada_1lam$selected_vars$pval
    pval_list[[5]]<-fuse_mul_ada$selected_vars$pval
    pval_list[[6]]<-fuse_mul_ada_1lam$selected_vars$pval

    nonzero_pos_list<-list()
    nonzero_pos_list[[1]]<-ada_res$nonzero_pos
    nonzero_pos_list[[2]]<-ada_res_lm$nonzero_pos
    nonzero_pos_list[[3]]<-fuse_uni_ada$selected_vars$position
    nonzero_pos_list[[4]]<-fuse_uni_ada_1lam$selected_vars$position
    nonzero_pos_list[[5]]<-fuse_mul_ada$selected_vars$position
    nonzero_pos_list[[6]]<-fuse_mul_ada_1lam$selected_vars$position
    names(nonpos)<-allname
    #names(nonpos0)<-allname
    names(powers)<-allname
    #names(powers0)<-allname
    names(coverage)<-allname
    nonpostotal[[i]]<-list(nonpos,powers,coverage,pval_list,nonzero_pos_list)
    print(c(nonpos))
    print(c(powers))
    print(c(coverage))
    if (i %% 5 == 0){
        saveRDS(nonpostotal,paste0(path,'Lnwrap_pos_ir_',inv_r,'_pX_',p_X,'_n_',ntest,'.rds'))
    }
}
