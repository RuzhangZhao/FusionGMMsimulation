...
for(i in c(1:6)){
    n<-ntest_list[i]
    #nonpos<-readRDS(paste0("Lgfinal_pos_ir_30_pX_40_n_",n,".rds"))
...
cn<-function(df1){
    ...
    df1$TLGMM[df1$TLGMM == 'ada']<-'Main Study w/ AdaLasso'
    df1$TLGMM[df1$TLGMM == 'mul_ada_1lam']<-'HTGMM w/ AdaLasso'
...
p<-ggplot(data=df1, aes(x=n, y=FDR,fill=TLGMM)) +
    ...
    geom_bar(stat="identity", position=position_dodge(),width = 0.6)+theme_bar()+
    scale_x_discrete(breaks = df1$n,labels =n_x,name = "Internal Sample Size")
ggsave(paste0("/users/rzhao1/intgmm/ln_fdr.pdf"),p,height = 4,width = 7)
...
p<-ggplot(data=df3, aes(x=n, y=CoverP,fill=TLGMM)) +
  ...
  geom_bar(stat="identity", position=position_dodge(),width = 0.6)+theme_bar()+
  scale_x_discrete(breaks = df3$n,labels =n_x,name = "Internal Sample Size")
ggsave(paste0("/users/rzhao1/intgmm/ln_cover.pdf"),p,height = 4,width = 7)
...
df2<-data.frame(df2)
df2 = df2[-c(1:2),]
colnames(df2)<-c("Method","Power","n")
df2$n<-round(as.numeric(df2$n)/30)
df2$Power<-as.numeric(df2$Power)
df2$n<-factor(df2$n,levels = sort(unique(df2$n)))
df2<-cn(df2)
n_x<-c(700,1000,1350,1700,2000)
p<-ggplot(data=df2, aes(x=n, y=Power,fill=Method)) +
  #geom_hline(yintercept=0.95, linetype="dashed", color = "red")+
  coord_cartesian(ylim = c(0,.75))+
  #labs(title = "Logistic: Power: TLGMM-adaptivelasso")+
  labs(title = "Linear: Power: HTGMM-adaptivelasso")+
  xlab("Internal Sample Size")+ylab("Power")+
  geom_bar(stat="identity", position=position_dodge(),width = 0.6)+theme_bar()+
  scale_x_discrete(breaks = df2$n,labels =rep(n_x,each=2),name = "Internal Sample Size")
ggsave(paste0("/users/rzhao1/intgmm/ln_power.pdf"),p,height = 4,width = 7)
...
p<-ggplot(data=df1, aes(x=n, y=FDR,fill=TLGMM)) +
    ...
    geom_bar(stat="identity", position=position_dodge(),width=0.6)+theme_bar()+
    scale_x_discrete(breaks = df1$n,labels =n_x,name = "Internal Sample Size")
ggsave(paste0("/users/rzhao1/intgmm/lg_fdr.pdf"),p,height = 4,width = 7)

df1[,2]<-c(0.79,0.86,0.874,0.915,0.944)
...
p<-ggplot(data=df1, aes(x=n, y=FDR,fill=TLGMM)) +
    ...
    geom_bar(stat="identity", position=position_dodge(),width=0.6)+theme_bar()+
    scale_x_discrete(breaks = df1$n,labels =n_x,name = "Internal Sample Size")
ggsave(paste0("/users/rzhao1/intgmm/lg_cover.pdf"),p,height = 4,width = 7)
