...
for(id1 in c(1:2)){
    for(id2 in c(1:2)){
        for(id3 in c(id3s)){
            ...
            if(!file.exists(paste0('/users/rzhao1/intgmm/result_desparseC/Lg/Lg_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))){
                rr_gap<-list()
            }else{
                rr_gap<-readRDS(paste0('/users/rzhao1/intgmm/result_desparseC/Lg/Lg_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
            }
            ...
            for(i in (length(rr_gap)+1):50){
                ...
                fuse_uni<-fusionGMM.binary.addition(X_int,A_int,1,y_int,sum_uni,penalty_type = "lasso",summary_type = "uni",approx_cross_validation =F,desparseC = desparseC)
                fuse_uni_1lam<-fusionGMM.binary.addition(X_int,A_int,1,y_int,sum_uni,penalty_type = "lasso",summary_type = "uni",approx_cross_validation =F,tune_ratio = F,desparseC = desparseC)
                cat("20%")
                fuse_uni_ada<-fusionGMM.binary.addition(X_int,A_int,1,y_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",approx_cross_validation =F,desparseC = desparseC)
                fuse_uni_ada_1lam<-fusionGMM.binary.addition(X_int,A_int,1,y_int,sum_uni,penalty_type = "adaptivelasso",summary_type = "uni",approx_cross_validation =F,tune_ratio = F,desparseC = desparseC)
                cat("40%")
                fuse_mul<-fusionGMM.binary.addition(X_int,A_int,1,y_int,sum_mul,penalty_type = "lasso",summary_type = "multi",approx_cross_validation =F,desparseC = desparseC)
                fuse_mul_1lam<-fusionGMM.binary.addition(X_int,A_int,1,y_int,sum_mul,penalty_type = "lasso",summary_type = "multi",approx_cross_validation =F,tune_ratio = F,desparseC = desparseC)
                cat("60%")
                fuse_mul_ada<-fusionGMM.binary.addition(X_int,A_int,1,y_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",approx_cross_validation =F,desparseC = desparseC)
                fuse_mul_ada_1lam<-fusionGMM.binary.addition(X_int,A_int,1,y_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",approx_cross_validation =F,tune_ratio = F,desparseC = desparseC)
                cat("80%")
                ...
                if (i %% 5 == 0){
                    saveRDS(rr_gap,paste0('/users/rzhao1/intgmm/result_desparseC/Lg/Lg_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
                }
            }
        }}}
