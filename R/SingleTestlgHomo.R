...
My_ext<-generateM(n_ext+n_int,p_X,p_A,coefXA,intercept)
M_ext<-My_ext$M
y_ext<-c(My_ext$y)
colnames(M_ext)<-XAcolnames
ids<-createDataPartition(y_ext,p=n_int/(n_int+n_ext))$Resample1
M_int<-M_ext[ids,]
y_int<-y_ext[ids]
M_ext<-M_ext[-ids,]
y_ext<-y_ext[-ids]
...
A_int<-M_int[,-c(1:p_X)]
library(intgmm)
fuse_uni_1lam<-cv.intgmm(y_int,X_int,A_int,sum_uni,family = "binomial",penalty_type = "lasso",summary_type = "uni",tune_ratio = F,use_sparseC=T)
fuse_mul_1lam<-cv.intgmm(y_int,X_int,A_int,sum_mul,family = "binomial",penalty_type = "lasso",summary_type = "multi",tune_ratio = F,use_sparseC=T)
