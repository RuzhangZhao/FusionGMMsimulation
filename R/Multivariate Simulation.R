...
id2 = as.numeric(args[2])
id3 = as.numeric(args[3])
p_X_list<-c(10,40)
...
coef_X[X_nonzero_index]<-0.17
coef_A[A_nonzero_index]<-0.2
...
for(i in 1:20){
    ...
    cat("95%\n")
if(F){
    betagap<-c(sum((coef(fuse_glm,s="lambda.min")[-1] - coefXA)^2),
      sum((fuse_mul$beta - coefXA)^2),
      sum((fuse_mul_app$beta - coefXA)^2),
      sum((fuse_uni$beta - coefXA)^2),
      sum((fuse_uni_app$beta - coefXA)^2),
      sum((fuse_mul_1lam$beta - coefXA)^2),
      sum((fuse_mul_1lam_app$beta - coefXA)^2),
      sum((fuse_uni_1lam$beta - coefXA)^2),
      sum((fuse_uni_1lam_app$beta - coefXA)^2)
      )
    names(betagap)<-c("glm","mul","mulap","uni","uniap","mul1","mul1ap","uni1","uni1ap")
}
    r0<-R2(y0,M0%*%coefXA)
    ...
    rr<-c(r0,r0X,r0A,rX,rlasso,rada,rmul,rmul1,rmulada,rmulada1,runi,runi1,runiada,runiada1,rmulap,rmul1ap)
    names(rr)<-c("r0","r0X","r0A","rX","rlasso","rada","rmul","rmul1","rmulada","rmulada1","runi","runi1","runiada","runiada1","rmulap","rmul1ap")
    print(round(rr,3))
    rr_gap[[i]]<-list("rr"=rr)
    if (i %% 4 == 0){
        saveRDS(rr_gap,paste0('/users/rzhao1/fusion/Ln3/Ln_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
    }
}
...
if(F){
  ...
  fuse_mul_ada_1lam<-fusionGMM.addition(X_int,A_int,y_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",approx_cross_validation =F,tune_ratio = F)
  cat("60%")
  fuse_mul_ridge<-fusionGMM.addition(X_int,A_int,y_int,sum_mul,penalty_type = "ridge",summary_type = "multi",approx_cross_validation =F)
  fuse_mul_init2<-fusionGMM.addition(X_int,A_int,y_int,sum_mul,penalty_type = "lasso",summary_type = "multi",approx_cross_validation =F,initial_with_GMM = F)
  cat("80%")
  fuse_mul_1lam_init2<-fusionGMM.addition(X_int,A_int,y_int,sum_mul,penalty_type = "lasso",summary_type = "multi",approx_cross_validation =F,initial_with_GMM = F,tune_ratio = F)
  #fuse_mul_ada_init2<-fusionGMM.addition(X_int,A_int,y_int,sum_mul,penalty_type = "adaptivelasso",summary_type = "multi",approx_cross_validation =F,initial_with_GMM = F)
  #fuse_mul_ridge_init2<-fusionGMM.addition(X_int,A_int,y_int,sum_mul,penalty_type = "ridge",summary_type = "multi",approx_cross_validation =F,initial_with_GMM = F)

  fuse_ols<-lm(y~.,data = data.frame(y=y_int,M_int))
  fuse_lasso<-cv.glmnet(x = M_int,y = y_int)
  fuse_ridge<-cv.glmnet(x = M_int,y = y_int,alpha=0)
  w_adaptive[is.infinite(w_adaptive)]<-max(w_adaptive[!is.infinite(w_adaptive)])*100
  fuse_ada<-cv.glmnet(x = M_int,y = y_int,penalty.factor = 1/abs(c(fuse_ols$coefficients[-1]))^(1/2))
  cat("100%")
  if(F){
    betagap<-c(sum((coef(fuse_glm,s="lambda.min")[-1] - coefXA)^2),
               sum((fuse_mul$beta - coefXA)^2),
  ...
  rX<-R2(y0,M0[,1:p_X]%*%sum_mul[[1]]$Coeff)
  rlasso<-R2(y0,M0%*%coef(fuse_lasso,s="lambda.min")[-1])
  rridge<-R2(y0,M0%*%coef(fuse_ridge,s="lambda.min")[-1])
  rada<-R2(y0,M0%*%coef(fuse_ada,s="lambda.min")[-1])
  ...
  rmulada<-R2(y0,M0%*%fuse_mul_ada$beta)
  rmulada1<-R2(y0,M0%*%fuse_mul_ada_1lam$beta)
  rmulridge<-R2(y0,M0%*%fuse_mul_ridge$beta)
  rmul2<-R2(y0,M0%*%fuse_mul_init2$beta)
  rmul12<-R2(y0,M0%*%fuse_mul_1lam_init2$beta)

  rmulap<-R2(y0,M0%*%fuse_mul_app$beta)
  rmul1ap<-R2(y0,M0%*%fuse_mul_1lam_app$beta)

  rr<-c(r0,r0X,r0A,rX,rlasso,rridge,rada,rmul,rmul1,rmulada,rmulada1,rmulridge,rmul2,rmul12)
  names(rr)<-c("r0","r0X","r0A","rX","rlasso","rridge","rada","rmul","rmul1","rmulada","rmulada1","rmulridge","rmul2","rmul12")
}
