...
for (id1 in c(3)){
  for(id2 in 1:2){
    p_X<-p_X_list[id1]
        ...
        rrid3 = rrid3[,c(1,2,4,6)]

        cm<-function(methods){
            methods2<-methods
            methods2[which(substr(methods,1,4)%in%c("runi"))]<-"HTLGMM w/ Lasso uni"
            methods2[which(substr(methods,1,4)%in%c("rmul"))]<-"HTLGMM w/ Lasso"
            methods2[which(substr(methods,1,4)%in%c("rlas","rada"))]<-"Internal Only w/ Lasso"
        ...
        #print(rrid3)
            ...
            if(gname == "Lasso"){
                df$Method<-factor(df$Method, levels = c("HTLGMM w/ Lasso","Internal Only w/ Lasso","ExternalOnly","HTLGMM w/ Lasso uni"))
            }
        ...
        }else if(p_X==150 & inv_r == 10){
          if(AUCR2=="AUC"){
            gg<-gg+labs(title = TeX(r'(Logistic AUC: $p_X=150,p_A=150,n_{ext}/n=10$)'))
          }else{
            gg<-gg+labs(title = TeX(r'(Linear $R^2$: $p_X=150,p_A=150,n_{ext}/n=10$)'))
          }
        }else if(p_X==150 & inv_r == 30){
          if(AUCR2=="AUC"){
            gg<-gg+labs(title = TeX(r'(Logistic AUC: $p_X=150,p_A=150,n_{ext}/n=30$)'))
          }else{
            gg<-gg+labs(title = TeX(r'(Linear $R^2$: $p_X=150,p_A=150,n_{ext}/n=30$)'))
          }
        }
     ...
     #   gg<-gg+scale_color_manual(values = c("#F8766D","#00BA38","#619CFF","#F564E3"))
        ...
        ggsave(paste0("/users/rzhao1/intgmm/finalplot/plot",lnlg2,gname,":",AUCR2,":Comp:Incomp=1:",inv_r,";RiskFactor:",p_X,".png"),gg,width = 7,height = 4.5)
  }
}
