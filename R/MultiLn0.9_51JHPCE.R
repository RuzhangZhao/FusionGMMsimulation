...
for(id1 in c(id1s)){
    for(id2 in c(id2s)){
        for(id3 in c(id3s)){
            ...
            if(!file.exists(paste0('/users/rzhao1/intgmm/result/Ln9/Ln9_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))){
                rr_gap<-list()
            }else{
                rr_gap<-readRDS(paste0('/users/rzhao1/intgmm/result/Ln9/Ln9_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
            }
            ...
            for(i in (length(rr_gap)+1):50){
                ...
                if (i %% 10 == 0){
                    saveRDS(rr_gap,paste0('/users/rzhao1/intgmm/result/Ln9/Ln9_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
                }
            }
        }}}
