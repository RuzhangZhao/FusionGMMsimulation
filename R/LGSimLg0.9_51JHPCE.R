    ...
    for(id2 in c(1:2)){
        ...
        for(id3 in c(id3s)){
            ...
            if(F){
                ...
                saveRDS(y0,paste0("/users/rzhao1/intgmm/data/test9binary_y_",p_X,".rds"))
            }else{
                #My0<-readRDS(paste0("/users/rzhao1/fusion/test/test2_",p_X,".rds"))
                My0<-readRDS(paste0("/users/rzhao1/intgmm/data/test9_",p_X,".rds"))
                M0<-My0$M
                y0<-readRDS(paste0("/users/rzhao1/intgmm/data/test9binary_y_",p_X,".rds"))
            }

            R2<-function(y0,predy){
                1-sum((predy-scale(y0,scale = F))^2)/sum((scale(y0,scale = F))^2)
            }
            if(!file.exists(paste0('/users/rzhao1/intgmm/result/Lg9/Lg9_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))){
                rr_gap<-list()
            }else{
                rr_gap<-readRDS(paste0('/users/rzhao1/intgmm/result/Lg9/Lg9_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
            }
            ...
            for(i in (length(rr_gap)+1):50){
                ...
                if (i %% 10 == 0){
                    saveRDS(rr_gap,paste0('/users/rzhao1/intgmm/result/Lg9/Lg9_n_',n_int,'_ir_',inv_r,'_pX_',p_X,'.rds'))
                }
            }
        }}}
